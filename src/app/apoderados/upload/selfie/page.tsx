...
import NeutralSelfie from "@/components/ui/icons/NeutralSelfie";
import { useApoderadoContext } from "@/context/ApoderadoContext"; // Asegúrate de importar el contexto
...
export default function Home() {
  ...
  const { setRenaper } = useApoderadoContext();

  const tokenProvisorio =
    "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJJZENsaWVudGUiOiIxMTk3MzQ1IiwiSWRBcG9kZXJhZG8iOiI4NzcxIiwic2NvcGUiOiJSLU9OQi1PTkJPQVJESU5HIn0.lm01p4YnKDoHpfyCzkxRSe9IvBgifsX0GM4kWlSUikk";
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (status === 200) {
        ...
        setRenaper(false); // Contexto
        handleUploadAnotherPhoto(); // Reiniciar selección de archivo y preview
      }
    } catch (error) {
      ...
      setRenaper(false); //
      setRenaper(false); //Contexto
      handleUploadAnotherPhoto(); // Reiniciar en caso de error
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <>
      <div className="mx-auto h-72 text-center relative w-[350px] 2xl:left-40 2xl:max-w-[90%]">
        ...
        <p className="text-gray-400 mt-2 text-sm">
          {intentos === 0 && !selectedFile
            ? "Sacate una selfie sin sonreir y subila:"
            : intentos === 1 && !selectedFile
      ...
      <div className="w-full flex justify-center relative top-20 mx-auto left-6 2xl:left-44">
        ...
        {selectedFile && photoPreview ? (
          ...
          <div className="flex w-[400px] h-28 justify-center  relative ">
            <button
            ...
            >
              ...
              {intentos >= 2 && !selectedFile ? (
                <Download className="relative right-5" />
              ) : null}
            </button>
          </div>
        )}
      ...
      {intentos >= 2 && !selectedFile ? (
        <div className="w-[300px] text-center text-sm relative top-16 left-6">
          <button
            onClick={handleContinueWithoutValidation}
            className="bg-transparent py-2 px-6 text-primary-500 flex justify-center gap-x-3 rounded-lg  h-10 max-w-[280px] border border-primary-500"
          >
