...
export const SelectIIBB = ({ defaultValue, onChange }: SelectProps) => {
  ...
  useEffect(() => {
    if (staticData) {
      const grossIncome = staticData.find(
        (item) => item.type === "IIBBType" // Cambiado a "IIBBType"
      );

      if (grossIncome && grossIncome.data) {
        const optionsArray = Object.entries(grossIncome.data).map(
          ([key, value]) => ({
            value: key,
