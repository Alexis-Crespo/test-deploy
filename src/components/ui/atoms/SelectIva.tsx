...
export const SelectIva = ({ defaultValue, onChange }: SelectProps) => {
  ...
  useEffect(() => {
    if (staticData) {
      const vatStatus = staticData.find((item) => item.type === "VatStatus");

      if (vatStatus && vatStatus.data) {
        const optionsArray = Object.entries(vatStatus.data).map(
          ([key, value]) => ({
            value: key,
